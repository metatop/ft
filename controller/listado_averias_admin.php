...
/**
 * Controlador de listado de averias admin
 *
 * @author Tiz
 */
...
class listado_averias_admin extends fs_controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Listado Averias Admin', 'Averias');
    }
    ...
    protected function private_core() 
   {
        ...
        if($this->query != '')
        {    
        $this->lista_averias = $this->tablaaverias->search($this->query, $this->offset);
        }   
        else
        $this->lista_averias = $this->new_message('Se puede buscar por modelo o chasis');
        
        if( isset($_GET['cod']) )
        {    
        $this->lista_averias = $this->tablaaverias->search($_GET['cod'], $this->offset);
        }   
    }
}
